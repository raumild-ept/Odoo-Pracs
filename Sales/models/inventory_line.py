...
class InventoryLine(models.Model):
    ...
    product_id = fields.Many2one(comodel_name='product.ept.sales',
                                 string='Product ID')
    inventory_id = fields.Many2one(comodel_name='stock.inventory.ept',
                                   string='Inventory ID', readonly=True)
    available_qty = fields.Float(digits=(6, 2), string='Available Quantity',
                                 readonly=True)
    counted_product_qty = fields.Float(digits=(6, 2),
                                       string='Counted Product Quantity')
