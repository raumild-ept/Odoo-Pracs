...
class ResPartnerEpt(models.Model):
    ...
    age_calculate = fields.Integer(compute="_com_age",search='_search_age',string='Age')
    ...
    def _com_age(self):
        for record in self:
            if record.bdate:
                    today = date.today()
                    record.age_calculate = today.year - record.bdate.year - ((today.month, today.day) < (record.bdate.month, record.bdate.day))
            else:
    ...
    def _search_age(self,opr,val):
        ...
        #     if opr == '=':
        #         if data.age_calculate == val:
        #             domain.append(data.id)
        sql = f"select id from res_partner_sales_ept where bdate is not null and date_part('year',age(now(),bdate)) {opr} {val};"
        ...
        for record in self:
            if record.bdate:
                if date.today().month < record.bdate.month:
                    record.age_calculate_readonly = date.today().year - record.bdate.year - 1
                else:
                    record.age_calculate_readonly = date.today().year - record.bdate.year
