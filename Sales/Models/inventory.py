...
class Inventory(models.Model):
    ...
    def start_inventory(self):
        ...
        for inventory_line in self.inventory_line_ids:
            inventory_line.available_qty = inventory_line.with_context(loc = self.location_id.id).product_id.product_stock
    ...
    def validate_inventory(self):
        ...
        for inventory_line in self.inventory_line_ids:
            if inventory_line.difference != 0:
                inventory_loss_loc=self.env['stock.location.ept'].search([('location_type','=','Inventory Loss')])
                if inventory_line.difference > 0:
                    location = [inventory_loss_loc.id, self.location_id.id]
                else:
                    location = [self.location_id.id, inventory_loss_loc.id]

                self.env['stock.move.ept'].create({'name':inventory_line.product_id.name,
